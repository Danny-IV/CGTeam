...
async function initPhongShader() {
    const vs = await readShaderFile('shVertPh.glsl');
    const fs = await readShaderFile('shFragPh.glsl');
    return new Shader(gl, vs, fs);
}

async function initGouraudShader() {
    const vs = await readShaderFile('shVertGr.glsl');
    const fs = await readShaderFile('shFragGr.glsl');
    return new Shader(gl, vs, fs);
}
